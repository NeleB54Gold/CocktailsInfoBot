...
if ($v->chat_type == 'private' and $v->isAdmin()) {
	...
	if ($v->command == 'setup') {
		if ($db) {
			$r = $db->setup();
			$r['cocktails'] = $db->query('CREATE TABLE IF NOT EXISTS cocktails (
				id				INT				PRIMARY KEY,
				name			VARCHAR(64)		NOT NULL,
				glass			VARCHAR(64),
				category		VARCHAR(64),
				ingredients		TEXT			NOT NULL	DEFAULT \'{}\',
				garnish			VARCHAR(64)		DEFAULT \'\',
				preparation		TEXT			DEFAULT \'\'
			);');
			$r['ingredients'] = $db->query('CREATE TABLE IF NOT EXISTS ingredients (
				id				INT				PRIMARY KEY,
				name			VARCHAR(64)		NOT NULL,
			...
			if (file_exists('data/recipes.json')) {
				...
				foreach ($recipes as $row) {
					$q = $db->query('INSERT INTO cocktails (id, name, glass, category, ingredients, garnish, preparation) VALUES (?,?,?,?,?,?,?)', [$id, $row['name'], $row['glass'], $row['category'], json_encode($row['ingredients']), $row['garnish'], $row['preparation']]);
					$id += 1;
				}
			}
			if (file_exists('data/ingredients.json')) {
				$ingredients = json_decode(file_get_contents('data/ingredients.json'), true);
				$id = 0;
				foreach ($ingredients as $name => $row) {
					$q = $db->query('INSERT INTO ingredients (id, name, alcohol, taste) VALUES (?,?,?,?)', [$id, $name, $row['abv'], $row['taste']]);
					$id += 1;
				}
			}
